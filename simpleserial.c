...
static int hex_decode(int len, char* ascii_buf, u8_t* data_buf)
{
	for(int i = 0; i < len; i++)
	{
...
static u8_t check_version(u8_t* v)
{
	return 0x00;
}
...
void simpleserial_get(void)
{
	...
	u8_t data_buf[MAX_SS_LEN];
	char c;

	// Find which command we're receiving
	c = uart_getc(DEBUG_PORT);
...
void simpleserial_put(char c, int size, u8_t* output)
{
	...
	{
		uart_putc(DEBUG_PORT, hex_lookup[output[i] >> 4 ]);
		uart_putc(DEBUG_PORT, hex_lookup[output[i] & 0xF]);
	}

	// Write trailing '\n'
	uart_putc(DEBUG_PORT, '\n');
	uart_flush_tx(DEBUG_PORT);
}
