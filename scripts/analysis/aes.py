...
class AES(object):
    ...
    def decrypt_diff(self, ciphertext_a, ciphertext_b):
        ...
        inv_shift_rows(cipher_state_a)
        inv_shift_rows(cipher_state_b)
        log[self.n_rounds][AES.Step.ShiftRows] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))
        inv_sub_bytes(cipher_state_a)
        inv_sub_bytes(cipher_state_b)
        log[self.n_rounds][AES.Step.SubBytes] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))

        for i in range(self.n_rounds - 1, 0, -1):
            log[i] = [None] * 4
            add_round_key(cipher_state_a, self._key_matrices[i])
            add_round_key(cipher_state_b, self._key_matrices[i])
            log[i][AES.Step.AddRoundKey] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))
            inv_mix_columns(cipher_state_a)
            inv_mix_columns(cipher_state_b)
            log[i][AES.Step.MixColumns] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))
            inv_shift_rows(cipher_state_a)
            inv_shift_rows(cipher_state_b)
            log[i][AES.Step.ShiftRows] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))
            inv_sub_bytes(cipher_state_a)
            inv_sub_bytes(cipher_state_b)
            log[i][AES.Step.SubBytes] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))

        log[0] = [None] * 4
        add_round_key(cipher_state_a, self._key_matrices[0])
        add_round_key(cipher_state_b, self._key_matrices[0])
        log[0][AES.Step.AddRoundKey] = xor_bytes(matrix2bytes(cipher_state_a), matrix2bytes(cipher_state_b))
        return log
